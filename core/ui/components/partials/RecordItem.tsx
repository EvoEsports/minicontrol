...
export default function RecordItem({ id = '', pos = '0 0', 'z-index': z = 1, size = '100 4', nickname = '', time = 0, rank = 0, highlight = false }) {
    ...
    const textcolor = highlight ? `fff` : `${colors.widget_text}`;

    let rankColor = colors.widget_text;

    let diffColor = highlight ? "fff" : colors.widget_text;
    let prefix = " ";
    if (rank > 1) {
        diffColor = highlight ? "fff" : "900";
        prefix = "+";
    }

    let formattedTime = formatTime(time, false);
    if (rank > 1) {
        formattedTime = formattedTime.replace("00:", "").replace(/^0/, "");
    }
    if (rank == 1) rankColor = 'db0e';
    if (rank == 2) rankColor = 'aabe';
    if (rank == 3) rankColor = '963e';
    rankColor = highlight ? "fff" : rankColor;
    ...
    return (
        <frame id={id} pos={pos} z-index="2">
            ...
            <label pos="5 -2" z-index="1" size="21.5 4" text={nickname} scale="0.85" halign="left" valign="center2" textsize="1" textfont={fonts.label} textcolor={textcolor} />
