...
export default class AdminPlugin extends Plugin {
    async onLoad() {
        ...
        tmc.addCommand("//prev", this.cmdPrev.bind(this), "Go to previous map");
    }
    ...
    async cmdPrev(login: string, args: string[]) {
        const map = tmc.maps.previousMap;

        if (!map) return tmc.chat("¤error¤No previous map", login);
        if (map == tmc.maps.currentMap) return tmc.chat("¤error¤Previous map is the same as the current map", login);
        ...
        try {
            ...
            tmc.chat("¤error¤Could not jump to previous map", login);
        }
    }
}
