...
export default class ForceMods extends Plugin {
    ...
    overrideMods: boolean = process.env.FORCEMODS_OVERRIDE_TRACKMOD?.toLowerCase() === "true";
    ...
    async onEndMap(data: any) {
        ...
        if (!this.environments.includes(nextMap.Environnement)) {
            tmc.cli(`$fa0[ForceMods] WARNING:¤white¤ Next environment ${nextMap.Environnement} unknown. Falling back to default mod.`);
        } else {
            nextModUrl = this.mods[nextMap.Environnement];
            tmc.debug(`[ForceMods] DEBUG: Setting next mod for environment ${nextMap.Environnement} to ${nextModUrl}.`);
        }
        if (this.overrideMods) {
            tmc.debug("[ForceMods] DEBUG: Forcing mod to be overwritten due to FORCEMODS_OVERRIDE_TRACKMOD");
        }
        tmc.server.send("SetForcedMods", this.overrideMods, [{ Env: nextMap.Environnement, Url: nextModUrl }]);
    }
}
