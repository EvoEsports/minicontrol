...
Key pieces:
- `Manialink` class — manages manialink lifecycle (render, display, hide, destroy) and integrates with the `forge` renderer to produce final XML.
- `forge` (see `forge.md`) — provides JSX helpers, hooks, and render utilities.
...
- `id` — unique autogenerated id; useful for debugging
- `display()` — show the manialink (first display triggers `tmc.ui.displayManialink`)
- `hide()` — hide it from players
- `destroy(hide = true)` — teardown and ask UI manager to remove references
- `render()` — produces the manialink XML (internal; uses `renderJsx` and runs hook commit phases)
- `cleanReferences()` — runs `disposeScript` and clears fields for GC
